...
void ComputeMV(vector<float> r)
{
    ...
    angle_resolution = 360.0f / range_size;

    for (size_t i = 0; i < range_size; i++)
    {
        // A non-zero value (true) if x is a NaN value; and zero (false) otherwise.
        // isinf A non-zero value (true) if x is an infinity; and zero (false)
        // otherwise.
        if (!std::isnan(r[i]) && !std::isinf(r[i])) //取出有效值
        {
            float scan_distance = r[i];
            int sector_index = std::floor((i * angle_resolution) / sector_value); //计算当前激光点在360度中属于哪个sector
            sector_index = std::clamp(sector_index, 0, 11); //限制在[0,11]之间
...
{
    float goal_sector =
        (int)(0 - (sector_value - sector_scale) + 360) % 360; //(0-(30-10)+360)%360 = 340,goal_sector = 340
    ...
    for (int i = 0; i < (sector_value - sector_scale) * 2 / angle_resolution; i++) // for(i=0;i<40;i++)
    {
        ...
        // {
        //     ROS_INFO("real_index: %d", real_index);
        //     ROS_INFO("scan_distance: %f", scan_distance);   
        // }
    }
    if (scan_distance < 0.1)
    {
...
float CalculDirection(Point2D &goal)
{
    ...
    float goal_ori = G_theta * 180 /
                     PI; //目标点相对uav的方向信息，即与y轴的夹角，正值代表在第一三现象，负值代表在二四现象。ENU坐标系下
    if (goal_ori < 0)
    {
        goal_ori += 360;
    }
...
void local_pos_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    if(!is_init)
    {
        init_x = current_local_pos.x();
        init_y = current_local_pos.y();
        init_z = current_local_pos.z();
        is_init = true;
    }
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber waypoint_sub =
        nh.subscribe<mavros_msgs::WaypointList>("/mavros/mission/waypoints", 100, waypoints_cb);
    ros::Subscriber homePos_sub =
        nh.subscribe<mavros_msgs::HomePosition>("/mavros/home_position/home", 100, home_pos_cb);
    ...
    ROS_INFO("Please set the waypoint in QGC before running this program.");
    ...
    while (ros::ok())
    {
        ...
        for (int index = 0; index < waypoints.waypoints.size(); index++)
        {
            ...
            ROS_INFO("goal gps: %f %f", goal_gps.x(), goal_gps.y());
            ROS_INFO("goal ecef: %f %f", goal_ecef.x(), goal_ecef.y());
            ROS_INFO("goal enu: %f %f", enu_offset.x(), enu_offset.y());
            ROS_INFO("goal local: %f %f", p.pose.position.x, p.pose.position.y);
            pose.push_back(p);
        }
        ...
        for (int i = 0; i < pose.size(); i++)
        {
            while (ros::ok())
            {
                ...
                if (direction >= -0.5)
                {
                    ...
                    pos_target.position.z = desire_z + init_z;

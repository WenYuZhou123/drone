...
ros::Subscriber swarm_trajs_sub_, other_odoms_sub_, emergency_stop_sub_,
    one_traj_sub_;
ros::Publisher swarm_trajs_pub_, other_odoms_pub_, emergency_stop_pub_,
    one_traj_pub_;
string tcp_ip_, udp_ip_;
int drone_id_;
double odom_broadcast_freq_;
char send_buf_[BUF_LEN], recv_buf_[BUF_LEN], udp_recv_buf_[BUF_LEN],
    udp_send_buf_[BUF_LEN];
...
enum MESSAGE_TYPE { ODOM = 888, MULTI_TRAJ, ONE_TRAJ, STOP } massage_type_;
...
int wait_connection_from_previous_drone(const int port, int &server_fd,
  ...
  char str[INET_ADDRSTRLEN];
  ROS_INFO("Receive tcp connection from %s",
...
int serializeMultiBsplines(const traj_utils::MultiBsplinesPtr &msg) {
  ...
  for (size_t i = 0; i < msg->traj.size(); i++) {
    total_len += sizeof(int32_t) + sizeof(int32_t) + sizeof(double) +
                 sizeof(int64_t) + sizeof(double);
    total_len += sizeof(size_t) + msg->traj[i].knots.size() * sizeof(double);
    total_len +=
        sizeof(size_t) + (3 * msg->traj[i].pos_pts.size()) * sizeof(double);
...
int serializeOdom(const nav_msgs::OdometryPtr &msg) {
  ...
  unsigned long total_len = 0;
  total_len = sizeof(size_t) + msg->child_frame_id.length() * sizeof(char) +
              sizeof(size_t) + msg->header.frame_id.length() * sizeof(char) +
              sizeof(uint32_t) + sizeof(double) + 7 * sizeof(double) +
              36 * sizeof(double) + 6 * sizeof(double) + 36 * sizeof(double);
      ...
      case MESSAGE_TYPE::STOP: {
        if (valread == sizeof(std_msgs::Empty)) {
          if (valread == deserializeStop(stop_msg_)) {
            ...
            ROS_ERROR(
                "Received message length not matches the sent one (1)!!!");
            continue;
          }
        }
...
int main(int argc, char **argv) {
  ...
  string sub_traj_topic_name = string("/drone_") + std::to_string(drone_id_) +
                               string("_planning/swarm_trajs");
  swarm_trajs_sub_ =
      nh.subscribe(sub_traj_topic_name.c_str(), 10, multitraj_sub_tcp_cb,
                   ros::TransportHints().tcpNoDelay());

  if (drone_id_ >= 1) {
    string pub_traj_topic_name = string("/drone_") +
                                 std::to_string(drone_id_ - 1) +
                                 string("_planning/swarm_trajs");
    swarm_trajs_pub_ = nh.advertise<traj_utils::MultiBsplines>(
        pub_traj_topic_name.c_str(), 10);
  }

  other_odoms_sub_ = nh.subscribe("my_odom", 10, odom_sub_udp_cb,
                                  ros::TransportHints().tcpNoDelay());
  other_odoms_pub_ = nh.advertise<nav_msgs::Odometry>("/others_odom", 10);

  // emergency_stop_sub_ = nh.subscribe("emergency_stop_broadcast", 10,
  // emergency_stop_sub_udp_cb, ros::TransportHints().tcpNoDelay());
  // emergency_stop_pub_ = nh.advertise<std_msgs::Empty>("emergency_stop_recv",
  // 10);

  one_traj_sub_ = nh.subscribe("/broadcast_bspline", 100, one_traj_sub_udp_cb,
                               ros::TransportHints().tcpNoDelay());

...
void pid_control()
{
		...
		if(flag_no_object || fabs(error_hgt)<=hgt_threshold)
		{
			...
			if(error_hgt>=0)
			{
				setpoint_raw.position.z = current_z_record + 0.65*hgt_threshold*0.001;
				printf("111setpoint_raw.position.z = %f\r\n",setpoint_raw.position.z); 
				
			}
			else
			{
				setpoint_raw.position.z = current_z_record - 0.65*hgt_threshold*0.001;
				printf("222setpoint_raw.position.z = %f\r\n",setpoint_raw.position.z);
			}

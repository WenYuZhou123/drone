...
void pid_control()
{
		...
		if(setpoint_raw.velocity.x < -0.6)  
		{
			setpoint_raw.velocity.x = -0.6;
		}
		else if(setpoint_raw.velocity.x > 0.6) 
		{
			setpoint_raw.velocity.x = 0.6;	
		}

		setpoint_raw.velocity.z = error_hgt * linear_z_p / 1000 + (error_hgt - last_error_hgt) * linear_z_d / 1000;
		if (setpoint_raw.velocity.z < -0.1)
		{
			setpoint_raw.velocity.z = -0.1;
		}
		else if (setpoint_raw.velocity.z > 0.1)
		{
			setpoint_raw.velocity.z = 0.1;
		}
		
		setpoint_raw.yaw_rate = error_x_angle*yaw_rate_p + (error_x_angle - last_error_x_angle)*yaw_rate_d;
		if(setpoint_raw.yaw_rate < -1)  
		{
			setpoint_raw.yaw_rate = -1;
		}
		else if(setpoint_raw.yaw_rate > 1) 
		{
			setpoint_raw.yaw_rate = 1;
		}				
		

		//没检测到目标，高度保持不变，使用位置闭环控制
		if(flag_no_object || fabs(error_hgt)<=500)
		{
			...
			if(temp_flag)
			{
				...
				printf("current_z_record11111 = %f\r\n",current_z_record); 
			}
